...
const session_add = (student_id, counsellor_id, specification, time, link)=>
{
    return new Promise((resolve, reject)=>
    {
        ...
        pool.getConnection((connection_error, connection)=>
        {
            ...
            const query = 'INSERT INTO counsellation_sessions (StudentID, CounsellorID, Counsellation_Specification, Counsellation_Time, Counsellation_Link) VALUES (?, ?, ?, ?, ?)';
            connection.query(query, [student_id, counsellor_id, specification, time, link], (queryError, results) => 
            {
                connection.release();
                if(queryError)
                {
...
const session_delete = (session_id) => 
{
    return new Promise((resolve, reject) => 
    {
        ...
        pool.getConnection((connection_error, connection) => 
        {
            ...
            const query = 'DELETE FROM counsellation_sessions WHERE Counsellation_Id=?';
